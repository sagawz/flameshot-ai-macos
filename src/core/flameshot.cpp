...
CaptureWidget* Flameshot::gui(const CaptureRequest& req)
{
    ...
    if (nullptr == m_captureWindow) {
        ...
        // A borderless screen-sized tool window stays on the active desktop;
        // showFullScreen() would create and switch to a separate macOS Space.
        // Create and configure the native window before showing it so macOS
        // never gets a chance to assign it to the application's old Space.
        configureMacCaptureWindow(m_captureWindow);
        m_captureWindow->show();
        configureMacCaptureWindow(m_captureWindow);
#else
