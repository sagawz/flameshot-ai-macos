...
namespace {
void showAnalysisMessage(QWidget* parent,
                         QMessageBox::Icon icon,
                         const QString& title,
                         const QString& text)
{
    Q_UNUSED(parent)
    // Never enter a nested modal event loop here. On macOS a modal child of
    // the high-level capture overlay can be moved behind that overlay while
    // continuing to block every input event until Escape is pressed.
    ...
    box->show();
#if defined(Q_OS_MACOS)
    configureMacCaptureWindow(box);
#endif
}

QProgressDialog* showRecognitionProgress(QWidget* parent)
{
    auto* progress = new QProgressDialog(
      QObject::tr("Recognizing selection…"), QString(), 0, 0, parent);
    progress->setCancelButton(nullptr);
    // Vision runs synchronously. Keep this status window non-modal so it
    // cannot remain as an invisible modal blocker before the result dialog.
...
void finishRecognitionProgress(QProgressDialog* progress)
{
    if (!progress) {
        return;
    }
  ...
  , m_toolSizeByKeyboard(0)
{
    ...
    if (fullScreen) {
        ...
        // Cover the current screen without entering native fullscreen. Native
        // fullscreen creates a separate macOS Space and moves the user away
        // from the desktop that they intended to capture.
        setWindowFlags(Qt::WindowStaysOnTopHint | Qt::FramelessWindowHint |
...
void CaptureWidget::initButtons()
{
    ...
    // Existing installations have an already persisted toolbar list. Keep the
...
void CaptureWidget::handleToolSignal(CaptureTool::Request r)
{
    switch (r) {
        ...
        case CaptureTool::REQ_DECREASE_TOOL_SIZE:
            setToolSize(m_context.toolSize - 1);
            break;
        case CaptureTool::REQ_TRANSLATE_SELECTION:
            translateSelection();
            break;
        case CaptureTool::REQ_RECOGNIZE_QR:
            recognizeQrSelection();
            break;
...
QImage CaptureWidget::selectedImageForAnalysis() const
{
    return m_context.selectedScreenshotArea().toImage();
}

void CaptureWidget::translateSelection()
{
    QProgressDialog* progress = showRecognitionProgress(this);
    QString error;
    const QString text =
      NativeVisionService::recognizeText(selectedImageForAnalysis(), &error);
    finishRecognitionProgress(progress);
    if (!error.isEmpty()) {
        showAnalysisMessage(
          this, QMessageBox::Warning, tr("Text recognition failed"), error);
        return;
    }
    m_aiService->run(AiService::Task::Translate, text, this);
}

void CaptureWidget::summarizeSelection()
{
    QProgressDialog* progress = showRecognitionProgress(this);
    QString error;
    const QString text =
    ...
    m_aiService->run(AiService::Task::Summarize, text, this);
}

void CaptureWidget::recognizeQrSelection()
{
    QProgressDialog* progress = showRecognitionProgress(this);
    QString error;
    const QStringList values = NativeVisionService::recognizeQRCodes(
      selectedImageForAnalysis(), &error);
    finishRecognitionProgress(progress);
    if (!error.isEmpty()) {
        showAnalysisMessage(
          this, QMessageBox::Warning, tr("QR recognition failed"), error);
        return;
    }
    if (values.isEmpty()) {
        showAnalysisMessage(this,
                            ...
                            tr("No QR code found"),
                            tr("No QR code was found in the selection."));
        return;
    }
    auto* dialog = new TextResultDialog(
      tr("QR code result"), values.join("\n\n"), nullptr);
